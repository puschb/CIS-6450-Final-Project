...
class VaeInversion:
    ...
    def init_prompt_fusion(self, prompt: List[str]):
        uncond_input = [""] * len(prompt)
        uncond_embeddings = self.model.text_encoder(uncond_input)
        text_embeddings = self.model.text_encoder(prompt)
        self.context = torch.cat([uncond_embeddings, text_embeddings])
        self.prompt = prompt
    ...
    def invert(self, image_path: List[str], prompt: List[str], offsets=(0, 0, 0, 0), verbose=False, num_inner_steps=10, num_epoch=1, early_stop_epsilon=1e-5, use_fusion=False):
        if use_fusion:
            self.init_prompt_fusion(prompt)
        else: 
    ...
    def eval_init(self, image_path: List[str], prompt_gt: List[str], offsets=(0, 0, 0, 0), verbose=True, trainer=None, use_fusion=False):
        if use_fusion:
            self.init_prompt_fusion(prompt_gt)
        else: 
