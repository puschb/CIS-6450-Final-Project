...
def compute_edit_complexity(prompt_src, prompt_tar, clip_model):
    """Measure semantic distance between prompts using CLIP"""
    with torch.no_grad():
        text_src = clip.tokenize([prompt_src]).cuda()
        text_tar = clip.tokenize([prompt_tar]).cuda()
        embed_src = clip_model.encode_text(text_src)
        embed_tar = clip_model.encode_text(text_tar)
        embed_src = embed_src / embed_src.norm(dim=-1, keepdim=True)
        embed_tar = embed_tar / embed_tar.norm(dim=-1, keepdim=True)
        distance = (1 - (embed_src @ embed_tar.T).item()) / 2
        return distance

def compute_image_complexity(image):
    """Measure image complexity via edge density"""
    img_gray = np.array(image.convert('L'))
    edges = ndimage.sobel(img_gray)
    complexity = np.mean(np.abs(edges)) / 255.0
    return min(1.0, complexity * 2)

def get_type_modifier(editing_type_id):
    """Different edit types need different step counts"""
    type_modifiers = {
        0: 0.8,  # Add attribute - simple
        1: 1.0,  # Replace object - medium
        2: 1.0,  # Add object - medium
        3: 0.9,  # Delete object - simple
        4: 1.1,  # Change attribute - complex
        5: 1.2,  # Change action - complex
        6: 1.0,  # Change color - medium
        7: 1.1,  # Alter parts - complex
        8: 1.3,  # Change background - very complex
        9: 1.4,  # Change style - very complex
    }
    return type_modifiers.get(editing_type_id, 1.0)

def compute_adaptive_steps(prompt_src, prompt_tar, input_image, editing_type_id, clip_model, base_steps=50):
    """Compute optimal step count based on complexity"""
    edit_complexity = compute_edit_complexity(prompt_src, prompt_tar, clip_model)
    image_complexity = compute_image_complexity(input_image)
    type_modifier = get_type_modifier(editing_type_id)
    
    adaptive_steps = int(base_steps * 
                        (0.6 + 0.4 * edit_complexity) *
                        (0.8 + 0.2 * image_complexity) * 
                        type_modifier)
    
    adaptive_steps = max(25, min(70, adaptive_steps))
    
    return adaptive_steps, edit_complexity, image_complexity, type_modifier
...
def edit_instruct_pix2pix(
    ...
    if edit_method=="instruct-pix2pix":
        ...
        # Phase 2: Adaptive step computation
        if use_adaptive and original_prompt:
            adaptive_steps, edit_comp, img_comp, type_mod = compute_adaptive_steps(
                original_prompt, edit, input_image_numpy, editing_type_id, CLIP_MODEL, base_steps=steps
            )
            print(f"  Adaptive: {steps}→{adaptive_steps} steps (edit:{edit_comp:.2f}, img:{img_comp:.2f}, type:{type_mod:.1f})")
